...
/** Rooms that are not included as their own section page (include === false); shown on Additional Sections. */
function getRollupRooms(
  snapshot: { rooms: { id: string; name: string; scopeNarrative?: string | null }[] },
  pages: PresentationConfigSaved["pages"] | undefined
) {
  const roomsConfig = pages?.rooms ?? {};
  const sectionsConfig = pages?.sections;
  const sectionsMap =
    sectionsConfig &&
  ...
  return snapshot.rooms.filter((room) => {
    const sectionCfg =
      sectionsMap && room.id in sectionsMap ? sectionsMap[room.id] : undefined;
...
export default async function AdditionalSectionsPage({
  params,
}: {
  ...
  return (
    <article className="space-y-8 pt-16">
      ...
      <p className="mt-4 text-zinc-600 dark:text-zinc-400">
        Additional spaces and scope. Select a room for detail.
      </p>
      {roomSlugs.length === 0 ? (
        <p className="mt-10 rounded-xl border border-zinc-200 bg-zinc-50 px-6 py-8 text-zinc-600 dark:border-zinc-700 dark:bg-zinc-800/50 dark:text-zinc-400">
