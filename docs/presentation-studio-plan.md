...
**⚠️ PRODUCTION RUNS A SEPARATE DATABASE from local dev** (verified 06-12: prod project ids don't exist in the dev Neon DB; prod env vars are Vercel-"Sensitive" = not pullable). Consequences: prod `BrandIcon` cache starts COLD (icons regenerate fresh, one-time per concept), prod has its own Integration rows (Anthropic + Gemini keys both work there) and its own CompanySettings; `vercel-build` runs `prisma migrate deploy` so schema stays in sync automatically. Day-1 prod fixes shipped 06-12: (1) `/api/jobs/studio-render` was missing from the Clerk `proxy.ts` public-route allowlist → QStash deliveries 404'd → studio renders stuck "Generating" (fixed + verified; remember: EVERY new QStash worker route must be added to `proxy.ts`); (2) AI Edit prompt hard-coded `regenerateIcons` in the JSON shape and omitted `regenerateIllustrations` → icon requests on Objective returned "didn't return an applicable change" (shape now built per-descriptor + regenerateIcons→"zones" alias); (3) all Gemini image bursts (objective hub/zones, scope/duotone icon resolvers, AI-edit illustrations) now run through `app/lib/async-pool.ts` `mapWithConcurrency(…, 2, …)` with one retry — the unbounded `Promise.all` bursts tripped rate limits on the cold prod cache (hub drew, all zones silently failed → vector fallbacks). Recovery for affected decks: re-run "Generate illustrations" (resumable, skips existing art).
