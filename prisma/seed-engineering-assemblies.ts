...
const ASSEMBLIES: AssemblySeed[] = [
  ...
  {
    slug: "edge-beam-at-post",
    name: "Edge Beam at Post (Porch Roof Carrying Beam)",
    category: "Roof Framing",
    discriminator: "porch-roof-carrying-beam",
    whenToUse:
      "The carrying beam/girder that spans between porch/pavilion columns (or posts) to support the porch ROOF rafters on the open sides. REQUIRED on any roofed structure carried on columns/posts — without it the rafters have no bearing between columns.",
    methodSummary:
      "Run a continuous edge beam (built-up wood per plan/span) along each open side, bearing on the corner columns/posts. PRIMARY connection: carefully notch the post so the beam bears on it with half the post continuing up the beam face, then through-bolt with (2) 5/8 in thru-bolts. ALTERNATE: attach each post/column to the beam above with a Simpson PC6Z post cap plus blocking as required.",
    codeBasis: "Gravity + wind uplift load path, ASCE-7/16.",
    quantityBasis: "Edge beam per LF of open side; connection (thru-bolts or PC6Z) per column/post.",
    caveats:
      "Heath drew a primary 'Edge Beam at Post' (notch + (2) 5/8 in thru-bolts) and an 'Alt. Edge Beam at Post' (Simpson PC6Z post cap) — both captured here as connection options. Pair with 'Porch Eave' for rafter bearing + sheathing.",
    triggerKeywords: [
      "roof-framing",
      "deck-porch",
      "porch",
    ...
    engineerName: "Adam W. Austin",
    engineerLicense: "SC PE No. 34907",
    sourceRef: "S101 (Heath — 'Edge Beam at Post' + 'Alt. Edge Beam at Post')",
    components: [
      { kind: "MEMBER", name: "Edge beam / girder", spec: "built-up wood beam per plan/span", qtyRule: "per LF of open side", notes: "carries porch roof rafters between columns" },
      { kind: "MEMBER", name: "Post / column", spec: "per plan (TD hollow box column or solid TD post)", qtyRule: "per column", notes: "supports the edge beam" },
      { kind: "CONNECTOR", name: "Beam-to-post through-bolts (primary)", spec: "(2) 5/8 in thru-bolts", qtyRule: "2 per beam-on-post connection", notes: "post notched; half of post continues up beam face" },
      { kind: "CONNECTOR", name: "Post cap (alternate)", model: "Simpson PC6Z", qtyRule: "1 per post", isConditional: true, notes: "alternate to notch + thru-bolt; add blocking as req'd" },
    ],
    sources: [
      { projectName: "38 Heath", sourceFirm: FIRM, engineerName: "Adam W. Austin", engineerLicense: "SC PE No. 34907", certNumber: "C03355", drawingDate: "March 12, 2026", status: "Working set (signed & sealed)", sourceRef: "S101 — Edge Beam at Post / Alt. Edge Beam at Post", designCriteria: DC.heath, deltaNotes: "Two source details merged: primary notch + (2) 5/8 in thru-bolt, and PC6Z post-cap alternate." },
    ],
  },
  {
    slug: "porch-eave",
    name: "Porch Eave (Rafters Bearing on Edge Beam at Columns)",
    category: "Roof Framing",
    discriminator: "porch-eave-on-edge-beam",
    whenToUse:
      "Eave of a PORCH/pavilion roof where rafters bear on a continuous edge beam carried by hollow load-bearing columns — roof sheathing fastening, rafter tie-down, and the column-to-beam uplift tie. Use on screened-porch / covered-porch roofs on columns.",
    methodSummary:
      "Roof sheathing is 19/32 in OSB fastened with 8d common nails OR Simpson RSRS-01 (2.375 in long, 1.5 in serrated shank, .113 dia) at 6 in o.c. field and 4 in o.c. within 48 in of eaves/rakes/valleys/ridges, fastened to the continuous edge beam. Connect each rafter to the edge beam with a Simpson H2.5A. The porch is supported on a TD hollow load-bearing column; a threaded rod terminates on top of the beam above with a 1/4x3x3 washer + nut (tap threads to prevent back-off).",
    codeBasis: "Wind uplift, ASCE-7/16.",
    quantityBasis: "Sheathing per SF; H2.5A per rafter; column tie-down per column; sheathing fastener per SF / per LF of edge.",
    caveats: "Pair with 'Edge Beam at Post' (the carrying beam). Roof-sheathing thickness is 19/32 in OSB — do not substitute heavier sheathing.",
    triggerKeywords: [
      "roof-framing",
      "deck-porch",
      "porch",
    ...
    tags: ["simpson-h2.5a", "simpson-rsrs-01"],
    sourceFirm: FIRM,
    engineerName: "Adam W. Austin",
    engineerLicense: "SC PE No. 34907",
    sourceRef: "S201 / Detail 13 (Heath — Porch Eave)",
    components: [
      { kind: "MEMBER", name: "Roof sheathing", spec: "19/32 in OSB", qtyRule: "per SF", notes: "fasten to continuous edge beam" },
      { kind: "MEMBER", name: "Continuous edge beam", spec: "per plan", qtyRule: "per LF", notes: "see 'Edge Beam at Post'" },
      { kind: "MEMBER", name: "Hollow load-bearing column", spec: "TD hollow box column", qtyRule: "per column" },
      { kind: "CONNECTOR", name: "Roof sheathing fastener", model: "8d common OR Simpson RSRS-01 (2.375 in, 1.5 in serrated shank, .113 dia)", qtyRule: "6 in o.c. field; 4 in o.c. within 48 in of eaves/rakes/valleys/ridges" },
      { kind: "CONNECTOR", name: "Rafter-to-edge-beam", model: "Simpson H2.5A", qtyRule: "1 per rafter" },
      { kind: "CONNECTOR", name: "Column tie-down", spec: "threaded rod + 1/4x3x3 washer & nut", qtyRule: "1 per column", notes: "terminate on top of beam above; tap threads to prevent back-off" },
    ],
    sources: [
      { projectName: "38 Heath", sourceFirm: FIRM, engineerName: "Adam W. Austin", engineerLicense: "SC PE No. 34907", certNumber: "C03355", drawingDate: "March 12, 2026", status: "Working set (signed & sealed)", sourceRef: "S201 / Detail 13", designCriteria: DC.heath, deltaNotes: "Roof-sheathing fastening matches Typ. Eave; column/edge-beam/threaded-rod is the porch-specific content." },
    ],
  },
  {
